...
import numpy as np
import scipy.io
from Datasets.DatasetPreprocessing import prepare_dataset
...
class PairsDataset(Dataset):
    ...
    def __init__(self, device, fill_with_type):
        ...
        if (not os.path.exists(f'Data\ChineseDataset\PreparedDataset_{fill_with_type}')):
            os.mkdir(f'Data\ChineseDataset\PreparedDataset_{fill_with_type}')
        elif (len(os.listdir(f'Data\ChineseDataset\PreparedDataset_{fill_with_type}')) == 0):
            prepare_dataset(fill_with_type)


        self.df = pd.read_csv('Data\ChineseDataset\REFERENCE.csv', delimiter=',')
    ...
    def getEqPair(self, df, index):
        ...
        if (index >= 0) and (index < (df.shape[0])):
            ...
            ecg1 = scipy.io.loadmat(f'Data\ChineseDataset\PreparedDataset_{self.fill_with_type}\{df["Recording"][f_el_pos]}.mat')['ECG']
            ecg2 = scipy.io.loadmat(f'Data\ChineseDataset\PreparedDataset_{self.fill_with_type}\{df["Recording"][s_el_pos]}.mat')['ECG']
    ...
    def getDifPair(self, df1, df2, index):
        ...
        if (index >= 0) and (index < df1.shape[0]):
            ...
            random_ecg = scipy.io.loadmat(f'Data\ChineseDataset\PreparedDataset_{self.fill_with_type}\{df2["Recording"][rand_item]}.mat')['ECG']
            ecg = scipy.io.loadmat(f'Data\ChineseDataset\PreparedDataset_{self.fill_with_type}\{df1["Recording"][index]}.mat')['ECG']
